-- Self-contained restoration of the approved long-form warehouse guide.
CREATE TEMP TABLE "restore_warehouse" AS SELECT * FROM "PillarPage" WITH NO DATA;
INSERT INTO "restore_warehouse" (
...
VALUES (
  ...
  'Midpoint offers warehouse space to rent in Midrand for logistics, distribution, storage and adaptable industrial operations. The estate combines access to the Johannesburgâ€“Pretoria corridor with practical loading, yard, power and office configurations. Available buildings vary, so tenants should compare each unit against their vehicle movements, storage system, power demand, staffing and future growth plans.',
...
<p>Midpointâ€™s wider work environment includes on-site and nearby amenities that can support the people based at the estate. Restaurants, fitness facilities and outdoor spaces are relevant when assessing recruitment, staff retention, informal meetings and the everyday experience of teams who spend long shifts at the property. They do not replace sound warehouse specifications, but they form part of the complete occupancy decision.</p>
<p>Public transport connections, commuting patterns and shift times should be examined for the actual workforce. A central regional location can be valuable, but each occupierâ€™s staffing profile is different. Include key employees in the location assessment and test travel at the hours when shifts begin and end.</p>

<h2>Security, Access Control and Operational Risk</h2>
<p>Security requirements depend on product value, operating hours, insurance conditions and the number of vehicles and visitors entering the site. Review estate access control, the boundary, lighting, camera coverage, guard procedures and the security measures attached to the individual building. Clarify where the landlordâ€™s responsibility ends and where tenant-specific systems must begin.</p>
...
<p>Midpointâ€™s leasing team can provide current availability, available specifications and access for inspections. Because live inventory changes, this guide should be used together with the current vacancy schedule rather than as a promise that a particular size or configuration remains available.</p>
...
SET
  ...
  "updatedAt" = NOW()
FROM "restore_warehouse" AS source
